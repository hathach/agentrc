...
- Follow the contract's execution and completion procedure, including foreground or background handling, timeout guards and its blocking wait, called again while it reports the run still going; never impose an earlier cancellation, and never spend a tool call only to pass time or to check on a run.
