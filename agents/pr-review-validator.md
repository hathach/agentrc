...
- Harvest findings from every reviewer: match authors case-insensitively on the substrings `codex`, `copilot`, `coderabbit`, `claude` — logins differ across REST/GraphQL. The prompt names which reviewers auto-run on this repo; only those verdicts gate `done`, and without that list `done` stays false. Here is where each known one lives:
...
- Whether Claude reviews on its own depends on the repo's workflows; where it only answers an explicit `@claude` mention a PR may legitimately never get one, so unless the prompt lists Claude as auto-running, never hold `done` waiting for it. But when a Claude review or comment *is* there, harvest and adversarially validate its findings like any other reviewer's; an unresolved `valid` one still blocks `done`.
