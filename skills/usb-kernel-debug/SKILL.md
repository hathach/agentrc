...
Neither sees inside the device's firmware or the wire itself: pair this
with a wire sniffer (the `usb-sniffer` skill, if installed) and target-side
debugging (GDB on the MCU) for those.
...
**Hard limit — usbmon is URB-level, not wire-level.** It cannot show data toggle (DATA0/DATA1) or NAKs. A device-side stall and a toggle desync both look identical: Submits on an endpoint with no Completes. To tell them apart, pair usbmon with a wire sniffer, or with GDB on the device: read the endpoint control register (response/toggle bits) and the device stack's endpoint state at the moment of the hang.
...
```bash
sudo $S/usb_dyndbg.sh on usbcore xhci_hcd    # then reproduce while following `sudo dmesg -w`
sudo $S/usb_dyndbg.sh status                 # what is switched on right now
