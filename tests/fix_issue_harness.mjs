...
test('the happy path passes with the branch commits and a safe next step', async () => {
  ...
  assert.match(result.next, /^confirm the implement notes carry hook evidence, then one simplification challenge on a read-only coworker lane; Workflow \/validate \{"boards":\["stm32f407disco"\],"base":"abc1234","maxCycles":1,"skip":\["review","codex"\]\}; clean its artifacts/)
  ...
  assert.match(bare.result.next, /simplification challenge on a read-only coworker lane; no validation workflow named/)
  ...
  assert.match(unsupported.result.next, /simplification challenge on a read-only coworker lane; workflow \/full-check cannot run with repairs and its own reviews disabled \(does not forward maxCycles\) — launch its component stages/)
